...
- **初心** - 玩永劫无间时觉得界面挺有意思的，想试试用 WPF 还原出来
- **技术栈** - .NET 6、所有控件手写、没用第三方 UI 库
- **架构设计** - 经过精心设计，严格遵循 MVVM 模式，符合 WPF 工程规范，代码结构优雅
- **AI 助手** - 后来发现 VibeCoding 太香了，用来堆量实现功能
- **质量保证** - 项目一半人味一半 AI 味，但架构和核心设计不依靠 AI，是真刀真枪的工程
...
代码都是开源的，有些地方写得不够好，欢迎提 issue 或 PR。
...
架构上用了 **Prism** 做模块化，**DryIoc** 做依赖注入，整体是标准的 MVVM 模式。

界面完全手写，没用现成的 UI 库。动画效果、过渡效果这些都是一点点调出来的，力求还原游戏的感觉。

数据处理主要用 **Mapster** 做对象映射（比 AutoMapper 快），**Newtonsoft.Json** 处理 JSON（老牌好用）。

## 怎么运行

需要装 **Windows 10+** 和 **.NET 6 SDK**（或者更高版本）。

用 Visual Studio 2022 打开解决方案，直接运行 `NarakaBladepoint.App` 项目就行。
